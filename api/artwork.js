...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!title && !styleTags) {
      return res.status(400).json({ error: 'Missing track data for artwork generation.' });
    }

    // Build a rich visual prompt from the track data
...
visually striking, suitable for music streaming platforms.`;
    ...
    const googleUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-preview-image-generation:generateContent?key=${apiKey}`;
